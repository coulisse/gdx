...
void connection_loop(int *sk, struct timeval ts){
  ...
  while (1) {
      ...
      if (nready < 0) {
          perror(LBL_ERR_SELECT);
	  pthread_mutex_lock (&mutexLock);
          strcpy(thread_glb.msg,LBL_ERR_SELECT);
	  thread_glb.telnet_rc = ko;
	  pthread_mutex_unlock (&mutexLock);
          ...
          if (thread_glb.connection_status_request == login) {    //LOGIN --- we send the callsign at the first request
            gtk_label_set_text (widgets->g_lbl_connection_info,"XXXXXXXXXXXXXX");
	    pthread_mutex_lock (&mutexLock);
	    ...
	    pthread_mutex_unlock (&mutexLock);
	     ...
	     pthread_mutex_unlock (&mutexLock);
	      ...
	      pthread_mutex_lock (&mutexLock);
              strcpy(thread_glb.msg,"Connection closed by the remote end");
	      thread_glb.telnet_rc = ko;
	      pthread_mutex_unlock (&mutexLock);
              //return 0;
              return;
...
void* telnet(void* unused) {
    ...
    if (sock == -1) {
        ...
        pthread_mutex_lock (&mutexLock);
        strcpy(thread_glb.msg,"Could not create socket. Error");
	pthread_mutex_unlock (&mutexLock);
        //return 1;
        return NULL;
    }
    ...
    if (connect(sock , (struct sockaddr *)&server , sizeof(server)) < 0) {
        ...
        pthread_mutex_lock (&mutexLock);
        strcpy(thread_glb.msg,"connect failed. Error");
	pthread_mutex_unlock (&mutexLock);
        //return 1;
        return NULL;
    }
    puts("Connected...\n");
    pthread_mutex_lock (&mutexLock);
    strcpy(thread_glb.msg,"Connected...");
    pthread_mutex_unlock (&mutexLock);
